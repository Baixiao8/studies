...
## v10.0 · 全书从「综述」重写为「人话」+ voice 写作工程体系(2026-05-29)

> 这一版不改样式、不加功能,改的是**文字本身**——把 running-science 从「读着像 AI 综述」全 12 章重写成「像一个人在讲」,并沉淀出一套可复用的写作工程产线。

### 起因
读者反馈报告「很有人机感」。诊断:voice-lint 能过(没硬性 AI 指纹),但「姿态」还是综述——面面俱到不取舍、上帝视角没有人、数字堆公式。lint 抓不到这层。

### 1. voice 写作工程体系(存 baixiao-claude-workspace 私有仓库)
- `CORE.md`(5 条核心)+ `DICTIONARY.md`(细则)+ `SAMPLES.md`(好样本 + 反例库)
- `voice-lint.py`:6 条硬规则自动检测,已接进 `build.py`——每次 build 自动跑
- 三引擎:教练腔 / 同伴腔 / 格言腔(借 Mailchimp 的 X-but-not-Y、Pinker、NN/g)
- `RETRO.md` 方法论复盘 + `PROCESS.md` 工程纪律(8 坑 + 分层审查三步法)
- `memory/user.md` 事实白名单(防 sub-agent 编造用户经历)

### 2. running-science 全 12 章姿态重构
- **137K → 69K 字**(砍掉一半注水);EPUB 5.8M → 3.5M
- 每章定一个核心 + 一个带体温比喻贯穿(钟摆弹簧 / 物流线 / 家庭账本 / 修复欠债 / 合脚鞋码…)
- 数字从「公式」变「看懂」(CTL/ATL/TSB → 体能/疲劳/状态三个表;乳酸阈 → 浴缸)
- voice-lint 全章 0 硬违规
- 训练章(ch10)从 17 节训练科学教科书 → 8 节跑者指南

### 3. Mailchimp Voice and Tone 报告(II · UX & 产品区)
- 首份 newsprint 报刊风 + 写作工程实验

### 方法论沉淀
- **姿态重构产线**:1-2 章人亲手打样 → agent 批量模仿 → 人分层审(trust but verify)
- 「像人写」从一门手艺,变成一条带质检的产线
